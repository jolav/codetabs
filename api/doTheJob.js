...
function getTemp (req, res) {
  weatherData = {};
  // console.log(global.ct.config.openWeather.key)
...
function getFromGeo (req, res, format) {
  ...
  lib.makeHttpsRequest(getipurl, function (res2, data) {
    ...
    let gettempurl = 'https://api.openweathermap.org/data/2.5/weather?lat=' + weatherData.lat + '&lon=' + weatherData.lon + '&APPID=' + global.ct.config.openWeather.key;
...
function cityToTemp (req, res, city, format) {
  let path = base + city + '&APPID=' + global.ct.config.openWeather.key;

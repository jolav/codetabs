...
func routerGeoIP(w http.ResponseWriter, r *http.Request) {
	e = myError{}
	...
	format := strings.ToLower(path[0])
	if format == "json" || format == "xml" {
		stats.AddHit(c.App.Service, c.App.Mode, lib.GetIP(r), target)
		getGeoIP(w, format, target)
	} else {
		log.Println(fmt.Sprintf("Format %s not recognized", path[0]))
		e.Error = fmt.Sprintf("Format %s not recognized", path[0])
		lib.SendErrorToClient(w, e)
	}
}

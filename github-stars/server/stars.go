...
func getRepo(w http.ResponseWriter, r *http.Request, repo string) {
	e = myError{}
	stars = []*star{}
	data = []point{}
	x = graphqlStars{}
...
func takeOutData() {
	...
	for i, v := range responses { // sort responses
		var aux []*star
...
func doLinksRequests(links []string) {
	...
	for _, link := range links {
		go func(link string) {
			...
			req.Header.Add("Authorization", "token "+c.GitHub.Token[rndToken])
			client := &http.Client{
...
`, repo[0], repo[1])
	...
	rndToken := lib.GetRandomInt(0, 9)
	req.Header.Set("Authorization", "bearer "+c.GitHub.Token[rndToken])
